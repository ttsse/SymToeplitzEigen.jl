...
Computes the eigenvalues (and vectors) of the symmetric Toeplitz matrix `Tn`. The eigenvalues (and vectors) are first computed in "low" precision (specified using Low_pres_type with values of either Float32 or Float64) on the GPU. The calculated eigenvalues (and vectors) are then improved using an Iterative Refinement procedure in parallel up to a precision `Refinement_precision`, with a maximum of `Max_iter` iterations and with an error tolerance of `tol_fact*eps(BigFloat)`.
