...
function Refinement(
    ...
    if !(toeplitz_kernel in (:dense, :structured, :auto))
        throw(ArgumentError("Invalid toeplitz_kernel=$(toeplitz_kernel). Expected :dense, :structured, or :auto."))
    end
    ...
    use_toeplitz = effective_toeplitz_kernel == :structured
    ...
    Btmp_ws = [Matrix{Float64}(undef, n, n) for _ in 1:max_tid]
    r_ws = [DT.(zeros(Int64, n)) for _ in 1:max_tid]
    y_ws = [DT.(zeros(Int64, n)) for _ in 1:max_tid]
    yp_ws = [DT.(zeros(Int64, n)) for _ in 1:max_tid]
    r64_ws = [Vector{Float64}(undef, n) for _ in 1:max_tid]
    y64_ws = [Vector{Float64}(undef, n) for _ in 1:max_tid]
    x64_ws = [Vector{Float64}(undef, n) for _ in 1:max_tid]
    Bbig_ws = solve_mode == :fast ? Matrix{DT}[] : [DT.(zeros(Int64, n, n)) for _ in 1:max_tid]
    ...
    Threads.@threads :dynamic for ii in 1:m
        ...
        # Reuse buffers owned by the current worker thread.
        Btmp = Btmp_ws[tid]
        r = r_ws[tid]
        y = y_ws[tid]
        yp = yp_ws[tid]
        ...
        y64 = y64_ws[tid]
        x64 = x64_ws[tid]
        Bbig = solve_mode == :fast ? nothing : Bbig_ws[tid]
    ...
    if return_status
        status = (
            ...
            toeplitz_kernel_used = effective_toeplitz_kernel,
        )
